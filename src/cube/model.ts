...
// Worker-backed solver interface via Comlink
...
function ensureWorkerProxy() {
  ...
  const w = new Worker(new URL('./solverWorker.ts', import.meta.url), { type: 'module' })
  solverWorker = w
...
export async function generateSolverScramble(length = 25) {
  ensureSolver()
  const proxy = ensureWorkerProxy()
  return await proxy.scramble(length)
}
